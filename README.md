...
| [**session-retro**](./session-retro) | End-of-session review. Measures the session transcript rather than asking the model to recall it, which matters because a long session gets compacted. Reports active time, calls per tool, runs of the same tool back to back, exact repeats, and waits, then proposes a ranked short list and logs it to a backlog. Zero dependencies, reads only local files. | `./setup.sh` |
...
For the Codex-only `gpt-5-6-relay`, symlink its inner skill directory instead:
