...
name: calendar-plan
description: >
  Use when the user says "plan my day", "draft tomorrow's calendar",
  "run calendar-plan", "/calendar-plan", "schedule tomorrow", or wants to
  build a practical daily plan from Google Calendar, a Notion task page,
  Gmail signals, and a local Calendar Context note. Two modes:
...
Before each run, read `config/planning-preferences.md` in this skill directory. Treat that file as the source for user-specific calendar IDs, task source names, daily defaults, commute assumptions, and calendar routing. Keep this `SKILL.md` generic.
...
0. `config/planning-preferences.md`: configured accounts, source names, defaults, commute assumptions, and calendar routing.
...
- Load daily defaults from `config/planning-preferences.md` before placing flexible blocks.
...
- Add recurring preference blocks from `config/planning-preferences.md` even when the task source omits them.
...
- Use commute assumptions from `config/planning-preferences.md` when available.
...
- Create each block on the matching calendar from `config/planning-preferences.md` instead of defaulting to primary.
...
- When actual calendar edits show a repeated pattern, update `config/planning-preferences.md` instead of relying on memory alone.
