...
Assign this phase only after the user authorizes deployment. The Sol
coordinator gives a bounded Codex subagent the settled release plan; the child
does not invent a deployment strategy. If production evidence invalidates the
plan, stop the release and return the evidence to Sol for a new route.
