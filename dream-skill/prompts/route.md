...
Each candidate has an ordered `allowed_page_ids` list and may include `source_context` from the conversation's working directory. Treat that context as a strong prior for technical/project facts, but let explicit personal, health, or unrelated content override it. Resolve IDs through `page_catalog`, choose only a listed page, then choose an existing H2 section. If no listed page is a sound fit, return `gap`; if two remain equally plausible, return `ambiguous`.
