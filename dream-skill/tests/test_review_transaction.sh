...
decision() {
  ...
  jq -n \
    ...
    '{action:$action, mode:"replace", target:{vault:"test",page:"wiki/page.md",section:"Facts"}, old_content:$old, content:$new, candidate_confidence:"high", needs_review:true, rationale:"user correction"}' \
    > "$path"
}
...
"$SCRIPTS/vault-writer.sh" --vault "$VAULT" --page wiki/page.md --section Facts \
  --content 'Approved fact' --mode append --undo-log "$UNDO" --no-index-update
[ "$(wc -l < "$UNDO" | tr -d ' ')" = "1" ]
